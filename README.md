...
> 키를 아직 넣지 않아도 앱은 실행됩니다. 연동 기능만 비활성화된 상태로 표시되며,
> 키를 넣고 서버를 재시작하면 바로 동작합니다.
...
브라우저에서 http://localhost:5173 을 열고, 우측 상단 **구글 계정 연동** 버튼으로 로그인하면
캘린더 연동이 시작됩니다.
...
- To-Do·시간표·메모·설정은 브라우저(localStorage)에 저장됩니다. 캘린더 일정의 원본은 구글 캘린더입니다.
- 저장되는 OAuth 토큰은 디스크에 **암호화**되어 보관됩니다(AES-256-GCM). 여러 기기에서 토큰 파일을 공유하려면 `.env`의 `TOKEN_ENC_KEY`를 고정하세요.
