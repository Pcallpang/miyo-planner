 ...
 *  있는 칸에 드롭하면 기존 값을 채워서 보여준다(수정용) — 삭제는 그 칸을 휴지통으로
 *  드래그해서 한다. */
export default function MakeupDropForm({ top, left, initialSubject, initialRoom, onCancel, onSave }: Props) {
  ...
  return (
    <>
      ...
      >
        <p className="mb-2 text-xs font-semibold text-violet-600">{initialSubject ? '보강 수정' : '보강 추가'}</p>
