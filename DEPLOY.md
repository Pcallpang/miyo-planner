...
| `NEIS_API_KEY` | 로컬 `.env`의 값 (**비우면 급식·학사일정이 5건까지만 조회됩니다**) |
...
## 다른 선생님에게 열어줄 때 확인할 것

지금은 혼자 쓰는 상태입니다. 나중에 동료 선생님들에게 열어줄 때 필요한 내용만 미리 적어둡니다.

### 나이스(급식·학사일정) — 준비 끝났습니다

- 인증키는 **서버에 하나만** 있으면 되고 모든 사용자가 함께 씁니다. 선생님들이 각자
  발급받을 필요가 없습니다.
- 학교 검색은 **전국 대상**입니다. 각자 자기 학교를 검색해 고르면 되고, 고른 학교는
  사용자별로 따로 저장되므로 서로 섞이지 않습니다.
- 한 가지만 지켜보면 됩니다 — **나이스 일일 트래픽 한도**. 한 개의 키로 전원의 호출이
  몰리기 때문입니다. 사용량은 open.neis.go.kr 마이페이지에서 볼 수 있습니다.
  (호출을 줄이려고 급식·학사일정 응답에 6시간, 학교 검색에 하루짜리 브라우저 캐시를
  걸어두었습니다 — `server/routes/school.js`)

### 구글 로그인 — 여기가 진짜 관문입니다

이 앱은 구글 캘린더 권한을 쓰는데, 구글은 이를 **'민감한 범위'**로 분류합니다.
그래서 Google Cloud Console의 **OAuth 동의 화면 → 게시 상태**에 따라 쓸 수 있는 사람이 달라집니다.

| 게시 상태 | 누가 쓸 수 있나 | 준비 |
|---|---|---|
| **테스트** (기본값) | **미리 등록한 이메일 100명까지만**. 로그인이 7일마다 풀립니다 | 없음 — 오늘 바로 가능 |
| **프로덕션** | 누구나 | 구글 보안 심사 통과 필요 (몇 주) |

- **동료 몇 분~수십 명에게 열 때**는 '테스트' 상태 그대로가 가장 간단합니다.
  Google Cloud Console → OAuth 동의 화면 → **테스트 사용자**에 그분들 구글 이메일을
  추가하면 끝입니다. 등록되지 않은 계정은 로그인 단계에서 막힙니다.
- **누구나 가입해 쓰게 하려면** '프로덕션'으로 전환하고 구글 심사를 받아야 합니다.
  개인정보처리방침 페이지(이미 `client/public/privacy.html`로 준비되어 있습니다),
  공개된 홈페이지, 앱 시연 영상이 필요합니다.
